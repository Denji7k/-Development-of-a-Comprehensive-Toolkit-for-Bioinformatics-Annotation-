...
output_file = input("Enter the output file (leave blank to use input file name with '_output.txt' suffix): ").strip() or f"{input_file}_output.txt"
...
with open(output_file, 'w') as file:
    for seq_id, mass in sequence_masses.items():
        file.write(f"{seq_id}: {mass}\n")

print(f"Total masses of protein sequences have been written to {output_file}")
